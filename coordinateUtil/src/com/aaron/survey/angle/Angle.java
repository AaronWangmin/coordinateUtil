...
import com.aaron.survey.SurveyException;
...
public class Angle {
	...
	// 度分秒 --构造
	public Angle(double d,double m,double s) {
		this.angleValue = dms2Rad(d, m, s);
	}
	
	// 其它单位 --构造
	public Angle(double angleValue,ConstantHolder.ArcFormat arcFormat) {
		switch(arcFormat) {
		case RAD:
			this.angleValue = angleValue;
			break;
...
//		System.out.println(this.getDMS());
	 ...
	 * @return
	 */
	private static double rad2As(double rad) {
	 ...
	 * @return
	 */
	private static double as2Rad(double as) {
	 ...
	 * @return
	 */
	private static double dms2Rad(double d,double m,double s) {
